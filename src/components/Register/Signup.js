...
const Signup = () => {
    const { user, createUser, googleSignIn } = useContext(UserContext);
    ...
    return (
        <main className="w-full h-full flex flex-col items-center justify-center pt-2 pb-5 px-4">
            <div className="max-w-sm w-full text-gray-600 space-y-2">
                ...
                <p className="text-center">Already have an account? <Link to='/login' className="font-medium text-indigo-600 underline hover:text-indigo-500"> Please Login</Link></p>
            </div>
        </main>
    );
};
